...
A rack app is a Ruby object. This object sits between Rails and a server. Without a web framework like Rails, we can also create a rack app directly. That's what we will do here.

## The Big Picture

Before I dive into the technical details, it would help to illuminate how the HTTP process and rack application work. That will inform how we build our first rack app. Let's break down the basic process:
...
To begin this process, we have to choose the server we want to use. I'm using WEBrick for this exercise. Create a new ruby program called **app.rb**. The command to run a server, among other things, is part of the rack library. Require it.
...
Since our app is set to ```nil```, we must be expecting an error. I bet it's going to be instructive. When I tried to go to ```localhost:8080```, I got this error in my console.
...
```
If I restart my server and try to go to the port, I now get this response.
...
Let's provide for #call in our code, that takes an argument, and see what happens next. Since I know the server is going to pass the environment into #call, I'm going to ```puts``` it. It would help to also call the #inspect method on the environment to see what the data structure looks like. Here is **app.rb**:
...
Rack::Handler::WEBrick.run app
```
When restarting my server and going to ```localhost:8080```, I got a couple of golden nuggets. First, I see the output of my ```puts``` command:
...
```
The hash above is the environment, i.e., the parsed HTTP request that WEBrick is passing into my rack app. It has valuable information that we might be able to use. For example, ```"PATH_INFO"=>"/"``` and ```"REQUEST_METHOD"=>"GET"```.

But let's keep building this app. After my custom #call method performed ```puts``` on the environment, it proceeded to try to work with my app. I see an instructive error, which returns us to the path we're on.
...
- -> /
```
While this error is not completely informative, I know what it's trying to do. The server is attempting to parse an HTTP response. It's specifically looking for an array containing HTML. Let's cure that error. We're still being hacky, but we can have #call return the data that the server expects (a properly structured HTTP response), by doing the following:
...
I will re-run my server and hope for the best. What we want is the response code 200 that we put in our array and we also want the page to be properly rendered in our browser. Our server says:
